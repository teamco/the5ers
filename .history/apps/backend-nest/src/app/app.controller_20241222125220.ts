...
export class AppController {
   ...
   */
  async getStock(@Res() response, @Param('id') stockId: string) {
    try {
      const existingStock = await this.appService.getStock(stockId);
      return response.status(HttpStatus.OK).json({
        message: 'Student found successfully', existingStock,
      });
    } catch (err) {
