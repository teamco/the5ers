  ...
  const handleEditStock = (id: string) => {
    const onFinish = (values: IStock): void => {
      ...
      request(`http://localhost:3000/api/${id}`, { method: 'PUT', body: JSON.stringify(values) }).then(res => {
        ...
        setStocks(stocks.reduce((acc, s) => {
          if (s._id === id) {
            const { name, price, symbol, ...rest } = s; // Destructure only the necessary properties
            acc.push({ name, price, symbol, ...values, ...rest }); // Spread the rest of the properties
          } else {
            acc.push(s);
          }
          return acc;
