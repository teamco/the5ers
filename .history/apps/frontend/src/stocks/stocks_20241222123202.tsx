...
export function Stocks() {
  ...
  const handleEditStock = (id: string) => {
     ...
     * 
     * - Validates the form values and checks if there are any changes.
     * - Sets the loading state to true during the update process.
     * - Sends a PUT request to update the stock with the specified ID on the server.
     * - Displays a success notification upon successful update.
     * - Updates the local state with the new stock details.
     * - Resets the form and closes the modal after completion.
     * 
     * @param values - The updated stock values to be submitted.
     */
    ...
    const onFinish = (values: IStock): void => {
      ...
      request(`http://localhost:3000/api/${id}`, { method: 'PUT', body: JSON.stringify(values) }).then(res => {
        ...
        setStocks(stocks.map(s => s._id === id ? { ...s, ...values } : s));
    ...
    modalApi.info({
      width: 600,
      title: 'Update stock',
      content: <StockForm hadleCancel={hadleCancel} onFinish={onFinish} formRef={formRef} initialValues={stocks.find(s => s._id === id)} />,
      footer: null
    });
  }
  ...
  const handleCreateStock = () => {
    ...
    /**
     * Handles the form submission for creating a new stock.
     * 
     * - Validates the form values and checks if there are any changes.
     * - Sets the loading state to true during the create process.
     * - Sends a POST request to create a new stock on the server.
     * - Displays a success notification upon successful creation.
     * - Updates the local state with the new stock details.
     * - Resets the form and closes the modal after completion.
     * 
     * @param values - The new stock values to be submitted.
     */
    const onFinish = (values: IStock): void => {
      ...
      request(`http://localhost:3000/api`, { method: 'POST', body: JSON.stringify(values) }).then(res => {
        ...
        setStocks(stocks => [...stocks, res.newStock]);
